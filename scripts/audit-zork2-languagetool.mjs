...
const normalizedPrefixes = corrections.map(({ startsWith }) =>
  startsWith?.replace(/\s+/g, ' ').trim(),
);
const exactSources = new Set(corrections.map(({ source }) => source).filter(Boolean));
...
const reportPath = process.env.ZORK_LT_REPORT ?? '/tmp/zork2-languagetool-audit.json';
