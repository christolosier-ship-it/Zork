...
const files = ['2dungeon.zil', '2actions.zil', 'gverbs.zil'];
let count = 0;
for (const file of files) {
  const english = await readFile(`translations/zil/en/zork2/${file}`, 'utf8');
  const french = await readFile(`translations/zil/fr/zork2/${file}`, 'utf8');
...
console.log(
  `✓ Zork II : ${count} mots PSEUDO préservés et ligatures compatibles Z-machine v3.`,
);
