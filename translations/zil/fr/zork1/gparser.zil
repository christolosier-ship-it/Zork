			"Fichier PARSER générique pour La Trilogie ZORK commencé le 7/28/83 par MARC"
...
<CONSTANT O-PTR 0>	"pointeur de mot vers un jeton inconnu dans P-LEXV"
<CONSTANT O-START 1>	"pointeur de mot vers le début de la phrase dans P-LEXV"
<CONSTANT O-LENGTH 2>	"longueur en octets des jetons non analysés dans P-LEXV"
<CONSTANT O-END 3>	"pointeur d'octet vers le premier octet libre dans OOPS-INBUF"
...
<ROUTINE PARSER ("AUX" (PTR ,P-LEXSTART) WRD (VAL 0) (VERB <>) (OF-FLAG <>)
	...
	<COND (<ZERO? ,P-LEN> <TELL "Je vous demande pardon ?" CR> <RFALSE>)>
	<COND (<EQUAL? <SET WRD <GET ,P-LEXV .PTR>> ,W?OOPS>
	       <COND (<EQUAL? <GET ,P-LEXV <+ .PTR ,P-LEXELEN>>
		      ...
		      <SETG P-LEN <- ,P-LEN 1>>)>
	       <COND (<NOT <G? ,P-LEN 1>>
		      <TELL "Je ne peux pas empêcher votre maladresse." CR>
		      <RFALSE>)
		     (<GET ,OOPS-TABLE ,O-PTR>
		      <COND (<AND <G? ,P-LEN 2>
				  <EQUAL? <GET ,P-LEXV <+ .PTR ,P-LEXELEN>>
					  ,W?QUOTE>>
			     <TELL
"Désolé, vous ne pouvez pas corriger les erreurs dans le texte cité." CR>
			     <RFALSE>)
			    (<G? ,P-LEN 2>
			     <TELL
"Attention : seul le premier mot après OOPS est utilisé." CR>)>
		     ...
		     (T
		      ...
		      <TELL "Il n'y avait aucun mot à remplacer !" CR>
		      <RFALSE>)>)
	      (T
	       ...
	       <COND (<ZERO? <GETB ,OOPS-INBUF 1>>
		     ...
		     (<G? ,P-LEN 1>
		      <COND (<OR <EQUAL? <GET ,P-LEXV <+ .PTR ,P-LEXELEN>>
			    ...
			    (T
			     <TELL "Je n'ai pas compris cette phrase." CR>
			     <RFALSE>)>)
		     (T
		      <SET PTR <+ .PTR ,P-LEXELEN>>
		      <PUTB ,P-LEXV ,P-LEXWORDS
		       ...
		       <COND (<EQUAL? .WRD ,W?THEN ,W?PERIOD ,W?QUOTE>
			     ...
			     (<OR <SET VAL <WT? .WRD ,PS?PREPOSITION 0>>
			      ...
			      <COND (<AND <G? ,P-LEN 1>
				    ...
				    (<EQUAL? ,P-NCN 2>
				     <TELL
"Il y avait trop de noms dans cette phrase." CR>
				     <RFALSE>)
				    (T
				     <SETG P-NCN <+ ,P-NCN 1>>
				     <SETG P-ACT .VERB>
			     ...
			     (<AND <EQUAL? .VERB ,ACT?TELL>
				   ...
				   <EQUAL? ,WINNER ,PLAYER>>
			      <TELL
"Veuillez consulter votre manuel pour connaître la bonne façon de parler à d'autres personnes ou créatures." CR>
			      <RFALSE>)
			     (T
			      <CANT-USE .PTR>
			      <RFALSE>)>)
...
<ROUTINE SYNTAX-CHECK ("AUX" SYN LEN NUM OBJ
	...
	<COND (<ZERO? <SET VERB <GET ,P-ITBL ,P-VERB>>>
	       <TELL "Il n'y avait aucun verbe dans cette phrase !" CR>
	       <RFALSE>)>
...
"Cette phrase n'en est pas une. reconnaître." CR>
	...
	<COND (<AND .DRIVE1
	      ...
	      (<AND .DRIVE2
	       ...
	       <SYNTAX-FOUND .DRIVE2>)
	      (<EQUAL? .VERB ,ACT?FIND>
	       <TELL "On ne peut pas répondre à cette question." CR>
	       <RFALSE>)
	      (<NOT <EQUAL? ,WINNER ,PLAYER>>
	       <CANT-ORPHAN>)
	      (T
	       <ORPHAN .DRIVE1 .DRIVE2>
	       <TELL "Que voulez-vous ">
	       <SET TMP <GET ,P-OTBL ,P-VERBN>>
	       <COND (<EQUAL? .TMP 0> <TELL "dire">)
		     (<ZERO? <GETB ,P-VTBL 2>>
		      <PRINTB <GET .TMP 0>>)
		     (T
	       ...
	       <COND (<AND <ZERO? .LEN> .GCHECK>
		      <COND (.VRB
			     ...
			     <COND (<OR ,LIT <VERB? TELL ;WHERE ;WHAT ;WHO>>
				    ...
				    <SETG P-ADJN <>>
				    <RTRUE>)
				   (T <TELL "Il fait trop sombre pour voir !" CR>)>)>
		      <SETG P-NAM <>>
		      <SETG P-ADJ <>>
		      <RFALSE>)
			       ...
			       <COND (<AND <NOT <HELD? .OBJ>>
				      ...
				      <COND (<AND .TAKEN
					     ...
					     <COND (<EQUAL? .OBJ
							    ,NOT-HERE-OBJECT>
						    <TELL
"Vous n'avez pas ça !" CR>
						    <RFALSE>)>
					     <TELL "Vous n'avez pas le ">
					     <PRINTD .OBJ>
					     <TELL "." CR>
					     <RFALSE>)
					    (<AND <NOT .TAKEN>
...
<ROUTINE MANY-CHECK ("AUX" (LOSS <>) TMP)
	<COND (<AND <G? <GET ,P-PRSO ,P-MATCHLEN> 1>
	      ...
	      (<AND <G? <GET ,P-PRSI ,P-MATCHLEN> 1>
	       ...
	       <SET LOSS 2>)>
	<COND (.LOSS
	       <TELL "Vous ne pouvez pas utiliser plusieurs ">
	       <COND (<EQUAL? .LOSS 2> <TELL "dans">)>
	       <TELL "objets directs avec \"">
	       <SET TMP <GET ,P-ITBL ,P-VERBN>>
	       <COND (<ZERO? .TMP> <TELL "dire">)
		     (<OR ,P-OFLAG ,P-MERGED>
		      <PRINTB <GET .TMP 0>>)
		     (T
		      <WORD-PRINT <GETB .TMP 2> <GETB .TMP 3>>)>
